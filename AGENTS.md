...
Goal: Notepad-fast startup, Obsidian-pretty UI. Single self-contained executable.
...
- KaTeX + marked-katex-extension for synchronous LaTeX math rendering
- WebView2 (Edge) as the rendering engine (pre-installed on Win10/11)

## Build
Requires Node.js 24+ / npm at build time to prepare the embedded math assets.
```bash
...
- `scripts/build-math.mjs` — Packages pinned upstream math scripts, WOFF2 fonts and licenses, without a bundler
- `tests/math.test.cjs` — Focused Node built-in tests using the actual embedded browser assets
- `docs/math.md` — Openable math examples, syntax boundaries and update notes
...
- `src/frontend/` — All HTML/CSS/JS files (embedded at compile time via include_str!)
  ...
  - `math.css` — Scrollable display math and theme-aware errors
  - `vendor/math/` — Generated math scripts and font-embedded CSS (gitignored)
...
- Script load order: highlight.js → marked.js → KaTeX + marked-katex-extension → i18n.js → preview.js → tabs.js → editor.js → app.js
...
- Do not add DOM math auto-rendering or a custom TeX parser: tokenize via marked-katex-extension before Markdown rendering
- Keep npm math dependencies exactly pinned; update the lockfile and regenerate assets together
